...
- 시간표는 시각이 있는 항목이 없으면 시간축 대신 빈 상태를 사용한다. 일정이 있으면 현재 추천의 수업과 개인 일정만으로 세로 시간 범위를 계산한다. 시작은 가장 이른 일정이 속한 정각 30분 전이며 자정에서는 `00:00`으로 제한한다. 끝은 마지막 일정 종료보다 늦은 첫 정각이며 끝 경계에는 시간 레이블을 두지 않는다. 추천을 이동할 때 세로 스크롤은 맨 위로 돌아가고, 요일 열은 후보 간 폭이 흔들리지 않도록 모든 추천의 범위를 공유한다. 왼쪽 축과 가로 안내선은 교시가 아닌 30분 단위 실제 시각을 사용한다.
...
9. 동적 시간표 범위 — 해결: 현재 추천의 이른·늦은 일정을 보존하고, 가장 이른 일정이 속한 정각 30분 전부터 마지막 일정 종료보다 늦은 첫 정각까지 30분 실제 시간 축을 구성한다. 추천을 이동하면 세로 범위와 스크롤을 해당 추천에 다시 맞추되 요일 열은 후보 전체에서 공유한다. PNG는 각 후보의 시간·요일 범위를 독립적으로 다시 계산한다.
