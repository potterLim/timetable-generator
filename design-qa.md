...
- 시간표는 비어 있을 때 월–금과 11:00–19:00를 기본 범위로 사용하고, 일정에 따라 토요일·일요일과 더 이르거나 늦은 시각까지 확장한다. 일정이 있으면 가장 이른 일정이 속한 정각보다 30분 앞에서 시작하며, 추천 탐색 중에는 모든 추천의 합집합으로 축을 고정한다. 왼쪽 축과 가로 안내선은 교시가 아닌 30분 단위 실제 시각을 사용한다.
