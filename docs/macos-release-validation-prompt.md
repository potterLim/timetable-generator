...
- 삭제 취소
- 삭제 확인
- 추가·수정 중 시간표 편집 패널이 이유 없이 닫히지 않는지
- 시간표 편집 패널은 X 등 명시된 동작에서만 닫히는지
...
- 시각이 있는 항목이 없는 시간표는 시간축 대신 빈 상태 표시
- 30분 단위 가로선
- 실제 시간 표시
- 일정이 있으면 가장 이른 일정이 속한 정각보다 30분 앞에서 시작
- 00시대 일정은 `00:00`에서 시작하고 최소 하나의 시간 레이블 표시
- 마지막 일정이 끝난 시각보다 늦은 첫 정각에서 축 종료
- 축의 마지막 끝 경계에는 시간 레이블을 표시하지 않음
- 추천을 넘길 때 현재 추천의 범위로 축과 스크롤 위치를 다시 맞추는지
- 추천마다 주말 포함 여부가 달라도 화면의 요일 열과 열 폭은 후보 전체에서 안정적으로 유지되는지
- 월–금 열 폭 동일
...
- 1600 DIP 이상: 과목 패널 312 + 시간표 편집 288 inline
- 1280–1599: 과목 inline, 시간표 편집 304 overlay
- 1080–1279: 과목 inline 약 320, 시간표 편집 overlay
- 1080 미만: 양쪽 overlay
...
- 과목 찾기와 일정 목록 버튼 순서
- 과목 찾기가 열려 있으면 일정 목록이 도구 영역 가장 왼쪽에 오는지
- 과목 찾기와 일정 추가의 상하 위치
- 과목 패널 닫기
- 시간표 편집 열기
- 두 패널의 compact 상호 배제
- 시간표 편집을 열고 일정 추가·수정·과목 수정을 해도 패널이 임의로 닫히지 않는지
- 가운데 시간표를 클릭해도 유지되고 X로만 닫히는지
...
- 성공·실패 안내
- 빈 시간표에서 PNG 저장 동작이 노출되지 않는지
- 일정이 있으면 가장 이른 일정이 속한 정각보다 30분 앞에서 시작
- 마지막 일정이 끝난 시각보다 늦은 첫 정각에서 끝나고 해당 경계에는 시간 레이블이 없는지
- 모든 시간표를 한 번에 저장할 때 후보마다 시간·요일 범위를 독립적으로 다시 계산하는지
- 토요일
- 일요일 단독
- 개인 일정
- 긴 과목명
