...
캘린더 설명에는 학교명과 학기로 만든 사용자용 문장을 표시하고, 내부 소유권은 이벤트 URL의 v2 표식으로 관리합니다. 앱 관리 캘린더를 대체할 때는 유효한 v1·v2 소유 표식이 있는 일정만 삭제하고 v1 일정은 v2로 이전합니다. 사용자 일정과 소유 표식이 전혀 없는 이전 버전의 일정은 보존하므로, 소유 표식을 도입하기 전 버전에서 만든 캘린더를 처음 다시 내보내면 이전 일정이 한 번 중복될 수 있습니다.
