...
앱 관리 캘린더를 대체할 때는 Timetable Generator가 이벤트 URL에 남긴 소유 표식이 유효한 일정만 삭제합니다. 사용자 일정과 소유 표식이 없는 이전 버전의 일정은 보존합니다. 따라서 소유 표식을 도입하기 전 버전에서 만든 캘린더를 처음 다시 내보내면 이전 일정이 한 번 중복될 수 있으며, 배포 검증에서 이 마이그레이션 제한을 별도로 안내합니다.
