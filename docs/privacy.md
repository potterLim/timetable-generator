...
- Apple Calendar에 내보낸 캘린더와 앱이 만든 일정을 구분하는 로컬 관리 정보
- 마지막으로 검증한 전체 과목 카탈로그 캐시
- 화면 모드 설정(시스템 설정 사용·라이트·다크)

시간표 데이터와 Apple Calendar 로컬 관리 정보는 암호화되지 않은 JSON 파일로 저장됩니다.  
...
macOS에서 Apple Calendar 내보내기를 실행하면 앱은 운영체제의 캘린더 전체 접근 권한을 요청하고 Apple Calendar에 시간표를 반영합니다.  
Timetable Generator 자체는 이 과정에서 외부 서버에 접속하지 않지만 Apple Calendar 앱은 사용자의 iCloud·Google 등 macOS 계정 설정에 따라 동기화할 수 있습니다.  

주간 수업은 학기 종료일까지 반복되는 일정으로 저장합니다.  
앱이 만든 일정만 안전하게 갱신하기 위한 캘린더·일정 식별 정보는 기기의 앱 데이터 폴더에 저장하며, 일정의 제목·메모·URL처럼 Apple Calendar에 표시되는 항목에는 내부 식별 정보를 추가하지 않습니다.  
...
- Apple Calendar 접근 권한: macOS **시스템 설정 → 개인정보 보호 및 보안 → 캘린더**에서 Timetable Generator의 접근 권한을 끕니다.
...
문의하기 전 로컬 경로·계정 이름·시간표 내용 등 불필요한 정보를 제거해야 합니다.

## 11. 문의와 방침 변경

설치·사용 문제, 기능 제안, 개인정보 또는 보안 관련 문의는 [{{ site.support_email }}]({{ site.support_mailto }})으로 보내 주세요.  
사용 중인 운영체제와 앱 버전, 문제가 발생한 구체적인 상황과 오류 내용, 재현 과정을 알려 주면 확인에 도움이 됩니다. 가능하면 문제 화면의 스크린샷도 함께 보내 주세요.
...
화면 캡처나 파일에는 계정·인증 정보와 민감한 개인 일정 등 문제 확인에 필요하지 않은 정보를 포함하지 마세요.
