...
public sealed class MeetingScheduleTests
{
    ...
    public void ScheduledMeetingRequiresUniqueValidSlotsAndDefensivelyCopiesThem()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() => MeetingSchedule.CreateScheduled(Array.Empty<MeetingSlot>()));
        Assert.ThrowsExactly<ArgumentException>(() => MeetingSchedule.CreateScheduled(new MeetingSlot[] { mondayFirstPeriod, mondayFirstPeriod }));
        Assert.ThrowsExactly<ArgumentException>(() => MeetingSchedule.CreateScheduled(new MeetingSlot[] { default(MeetingSlot) }));
    }
    ...
    public void MeetingSlotRejectsUnsupportedDaysAndPeriods()
    {
        ...
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new MeetingSlot(EDay.None, firstPeriod));
