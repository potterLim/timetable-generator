...
public sealed class ShutdownOverlayInteractionTests
{
    ...
    public void SaveFailureKeepsBackgroundLockedUntilEditingContinues()
    {
        ...
        try
        {
            ...
            Grid productInteractionSurface = findRequiredControl<Grid>(hostWindow, "ProductInteractionSurface");
            Border shutdownOverlay = findRequiredControl<Border>(hostWindow, "ShutdownOverlay");
            Border shutdownDialog = findRequiredControl<Border>(hostWindow, "ShutdownDialog");
            Button appearanceButton = findRequiredControl<Button>(hostWindow, "AppearanceButton");
            Button continueEditingButton = findRequiredControl<Button>(hostWindow, "ContinueEditingButton");
            ...
            Assert.Equal(KeyboardNavigationMode.Cycle, KeyboardNavigation.GetTabNavigation(shutdownDialog));
            Assert.True(continueEditingButton.IsKeyboardFocusWithin);

            AutomationPeer continueEditingPeer = ControlAutomationPeer.CreatePeerForElement(continueEditingButton);
            IInvokeProvider continueEditingAction = Assert.IsAssignableFrom<IInvokeProvider>(continueEditingPeer);
