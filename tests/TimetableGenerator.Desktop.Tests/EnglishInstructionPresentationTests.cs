...
public sealed class EnglishInstructionPresentationTests
{
    ...
    public void CourseBrowserAndChoiceEditorRenderPercentageTextAndAccessibility()
    {
                ...
                CatalogProjectionTestFixture.CreateDocument()))
        {
            ...
            try
            {
                ...
                Assert.Equal(
                    course.CourseBrowserAccessibleName,
                    AutomationProperties.GetName(courseCard));
            }
            finally
            {
        ...
        IReadOnlyList<EnglishInstructionPercentage> percentages)
    {
        ...
        for (int index = 0; index < sourceCourse.Offerings.Count; ++index)
        {
            ...
            CatalogOfferingInstructionMetadata sourceInstruction =
                sourceMetadata.Instruction;
            ...
            offerings.Add(new CatalogOfferingProjection(
                sourceOffering.Offering,
                metadata));
        }

        return new CatalogCourseProjection(
            sourceCourse.Course,
            sourceCourse.Accent,
            offerings);
    }
