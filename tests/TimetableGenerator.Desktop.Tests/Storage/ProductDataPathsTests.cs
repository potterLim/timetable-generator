...
public sealed class ProductDataPathsTests
{
    ...
    {
        string rootPath = Path.Combine(Path.GetTempPath(), "TimetableGenerator.Tests", Guid.NewGuid().ToString("N"));
        ...
        Assert.Equal(Path.Combine(rootPath, "Settings", "appearance-v1.json"), paths.AppearanceSettings.Value);
