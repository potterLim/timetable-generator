...
public sealed class CalendarNameConflictPolicyTests
{
    ...
    public void NameMatchingUsesTrimmedUnicodeFormCAndOrdinalIgnoreCase()
    {
        ...
        Assert.True(
            CalendarNameConflictPolicy.IsNameInUse(
                composedName,
                new PlanName[] { decomposedName }));
    }
    ...
    public void NextAvailableNameRespectsThePlanNameLengthLimit()
    {
        ...
        Assert.Equal(
            new string('가', PlanName.MAXIMUM_LENGTH - " (2)".Length) + " (2)",
            nextAvailableName.Value);
    }
        ...
        for (int copyNumber = 2; copyNumber <= 10; ++copyNumber)
        {
            ...
            existingNames.Add(
                new PlanName(
                    new string(
                        'a',
                        PlanName.MAXIMUM_LENGTH - suffix.Length)
                    + suffix));
        }
