...
public sealed class WorkspaceEmptyStateTests
{
    ...
    public async Task WorkspaceShowsOnlyActionsRelevantToTheActivePlanAsync()
    {
        ...
        try
        {
            ...
            PlanCourseChoiceGroupItem scheduledCourseGroup = Assert.Single(workspace.ActivePlan.CourseChoiceGroups);
            ...
            workspace.CloseInspectorPaneCommand.Execute(null);
            Dispatcher.UIThread.RunJobs();
            assertEmptyScheduleState(scheduleWorkspace, true, "시간이 정해진 과목이 없습니다", "시간 미정 과목은 현재 시간표에 유지됩니다.");
            workspace.OpenInspectorPaneCommand.Execute(null);
            Dispatcher.UIThread.RunJobs();
    ...
    private static void assertEmptyInspectorState(PlanInspectorView planInspector)
    {
        ...
        Assert.Equal("과목을 선택해 시간표를 구성해 보세요.", description.Text);
        Assert.True(addPersonalScheduleButton.IsEffectivelyVisible);
        assertControlSharesHorizontalCenter(planInspector, emptyPlanStateContent, iconSurface);
        assertControlSharesHorizontalCenter(planInspector, emptyPlanStateContent, heading);
        assertControlSharesHorizontalCenter(planInspector, emptyPlanStateContent, description);
    ...
    private static void assertPopulatedInspectorState(PlanInspectorView planInspector)
    {
        Border emptyPlanState = findRequiredControl<Border>(planInspector, "EmptyPlanState");
        ListBox scheduledCourses = findRequiredControl<ListBox>(planInspector, "ScheduledCoursesList");
        Expander timeNotProvidedCourses = findRequiredControl<Expander>(planInspector, "TimeNotProvidedCoursesExpander");
        ...
        TextBlock emptyStateMessage = Assert.IsType<TextBlock>(personalScheduleEmptyState.Child);
        Assert.Equal("수업 외 고정 일정을 추가하세요.", emptyStateMessage.Text);
        Assert.Equal("수업 외 고정 일정을 추가하세요.", ToolTip.GetTip(emptyStateMessage));
    }

    private static void assertTimeNotProvidedChoiceInspectorState(PlanInspectorView planInspector)
    {
        Border emptyPlanState = findRequiredControl<Border>(planInspector, "EmptyPlanState");
        ListBox scheduledCourses = findRequiredControl<ListBox>(planInspector, "ScheduledCoursesList");
        Expander timeNotProvidedCourses = findRequiredControl<Expander>(planInspector, "TimeNotProvidedCoursesExpander");
