...
internal static class CourseCatalogProjector
{
    public static CourseCatalogProjection Project(CourseCatalogDocument document)
    {
        ...
        Dictionary<CourseId, List<CatalogOfferingProjection>> offeringsByCourseId =
            createEmptyOfferingGroups(document.Catalog.Courses);
        foreach (CatalogOffering offering in document.Catalog.Offerings)
        {
            ...
            CatalogOfferingProjection offeringProjection =
                new CatalogOfferingProjection(offering, metadata);

            List<CatalogOfferingProjection>? courseOfferingsOrNull;
            bool hasCourse = offeringsByCourseId.TryGetValue(
                offering.CourseId,
                out courseOfferingsOrNull);
        ...
        foreach (CatalogCourse course in document.Catalog.Courses)
        {
            ...
            courses.Add(new CatalogCourseProjection(
                course,
                accent,
                offeringsByCourseId[course.Id]));
        }

        return new CourseCatalogProjection(document, courses);
    }

    private static Dictionary<CourseId, List<CatalogOfferingProjection>>
        createEmptyOfferingGroups(IEnumerable<CatalogCourse> courses)
    {
        Dictionary<CourseId, List<CatalogOfferingProjection>> offeringsByCourseId =
            new Dictionary<CourseId, List<CatalogOfferingProjection>>();
