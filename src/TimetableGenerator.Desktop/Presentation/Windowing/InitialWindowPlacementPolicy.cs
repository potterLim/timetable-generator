...
internal static class InitialWindowPlacementPolicy
{
    ...
    private static readonly WindowLogicalSize PREFERRED_SIZE = new WindowLogicalSize(1_440.0, 900.0);

    private static readonly WindowLogicalSize DESIGN_MINIMUM_SIZE = new WindowLogicalSize(900.0, 640.0);

    public static InitialWindowPlacement CreatePlacement(WindowWorkingArea workingArea)
    {
        ...
        WindowLogicalSize initialSize)
    {
        ...
        int initialPixelWidth = (int)Math.Min(bounds.Width, initialPixelWidthValue);
        double initialPixelHeightValue = Math.Ceiling(initialSize.Height * workingArea.Scale.Value);
        int initialPixelHeight = (int)Math.Min(bounds.Height, initialPixelHeightValue);
