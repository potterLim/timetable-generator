...
public sealed class PlanningPlanContent
{
    ...
    public PlanningPlanContent(IEnumerable<CourseChoiceGroup> courseChoiceGroups, IEnumerable<UnscheduledOfferingSelection> unscheduledOfferingSelections, IEnumerable<PersonalSchedule> personalSchedules)
    {
        ...
        mCourseChoiceGroups = copyAndValidateCourseChoiceGroups(courseChoiceGroups, selectedCourseIds, selectedOfferingIds);
        mUnscheduledOfferingSelections = copyAndValidateUnscheduledOfferingSelections(unscheduledOfferingSelections, selectedCourseIds, selectedOfferingIds);
        mPersonalSchedules = copyAndValidatePersonalSchedules(personalSchedules);
    }

    private static IReadOnlyList<CourseChoiceGroup> copyAndValidateCourseChoiceGroups(IEnumerable<CourseChoiceGroup> courseChoiceGroups, ISet<CourseId> selectedCourseIds, ISet<OfferingId> selectedOfferingIds)
    {
        ...
        foreach (CourseChoiceGroup courseChoiceGroup in courseChoiceGroups)
        {
            ...
            {
                throw new ArgumentException("Planning plans cannot contain null course choice groups.", nameof(courseChoiceGroups));
            }

            if (groupIds.Add(courseChoiceGroup.Id) == false)
            {
                throw new ArgumentException("Planning plans cannot contain duplicate course choice group IDs.", nameof(courseChoiceGroups));
            }
            ...
            foreach (CourseCandidate courseCandidate in courseChoiceGroup.CourseCandidates)
            {
                ...
                {
                    throw new ArgumentException("Planning plans cannot select the same course more than once.", nameof(courseChoiceGroups));
                }
                ...
                foreach (OfferingCandidate offeringCandidate in courseCandidate.OfferingCandidates)
                {
                    ...
                    {
                        throw new ArgumentException("Planning plans cannot select the same offering more than once.", nameof(courseChoiceGroups));
                    }
                }
            }
    ...
    private static IReadOnlyList<UnscheduledOfferingSelection> copyAndValidateUnscheduledOfferingSelections(IEnumerable<UnscheduledOfferingSelection> unscheduledOfferingSelections, ISet<CourseId> selectedCourseIds, ISet<OfferingId> selectedOfferingIds)
    {
        ...
        foreach (UnscheduledOfferingSelection selection in unscheduledOfferingSelections)
        {
            if (selection == null)
            {
                throw new ArgumentException("Planning plans cannot contain null unscheduled selections.", nameof(unscheduledOfferingSelections));
            }

            if (selectedCourseIds.Add(selection.CourseId) == false)
            {
                throw new ArgumentException("A course cannot be both scheduled and time-unconfirmed in one plan.", nameof(unscheduledOfferingSelections));
            }

            if (selectedOfferingIds.Add(selection.OfferingId) == false)
            {
                throw new ArgumentException("Planning plans cannot select the same offering more than once.", nameof(unscheduledOfferingSelections));
            }
    ...
    private static IReadOnlyList<PersonalSchedule> copyAndValidatePersonalSchedules(IEnumerable<PersonalSchedule> personalSchedules)
    {
        ...
        foreach (PersonalSchedule personalSchedule in personalSchedules)
        {
            ...
            {
                throw new ArgumentException("Planning plans cannot contain null personal schedules.", nameof(personalSchedules));
            }

            if (scheduleIds.Add(personalSchedule.Id) == false)
            {
                throw new ArgumentException("Planning plans cannot contain duplicate personal schedule IDs.", nameof(personalSchedules));
            }
    ...
    private static void ensureScheduleDoesNotOverlap(IEnumerable<PersonalSchedule> existingSchedules, PersonalSchedule candidateSchedule)
    {
        foreach (PersonalSchedule existingSchedule in existingSchedules)
        {
            foreach (WeeklyTimeRange existingRange in existingSchedule.TimeRanges)
            {
                foreach (WeeklyTimeRange candidateRange in candidateSchedule.TimeRanges)
                {
                    ...
                    {
                        throw new ArgumentException("Personal schedules in one plan cannot overlap.", nameof(candidateSchedule));
                    }
                }
            }
        }
