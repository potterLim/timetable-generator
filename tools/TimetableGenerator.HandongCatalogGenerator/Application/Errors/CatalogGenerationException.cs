...
internal sealed class CatalogGenerationException : Exception
{
    ...
    public CatalogGenerationException(
        ECatalogGenerationErrorCode errorCode,
        ECatalogGeneratorExitCode exitCode,
        string message)
        ...
        : base(message, innerExceptionOrNull)
    {
        if (errorCode == ECatalogGenerationErrorCode.None)
        {
            throw new ArgumentOutOfRangeException(
                nameof(errorCode),
                errorCode,
                "A catalog generation exception must have an error code.");
        }
        ...
        if (exitCode == ECatalogGeneratorExitCode.Succeeded)
        {
            throw new ArgumentOutOfRangeException(
                nameof(exitCode),
                exitCode,
                "An exception cannot use the successful exit code.");
        }
